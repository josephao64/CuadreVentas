document.addEventListener("DOMContentLoaded", async () => {
    ...
    const mostrarCuadresRealizados = async () => {
        const cuadresTarjetas = document.getElementById('cuadres-sucursales');
        cuadresTarjetas.innerHTML = '';
        const querySnapshot = await db.collection("cuadres").orderBy("idCuadre", "desc").get();
    ...
    window.filterCuadres = async () => {
        ...
        if (searchId) {
            const querySnapshot = await db.collection("cuadres").where("idCuadre", "==", parseInt(searchId)).get();
            const cuadresTarjetas = document.getElementById('cuadres-sucursales');
            cuadresTarjetas.innerHTML = '';
            querySnapshot.forEach((doc) => {
                const data = doc.data();
